...
export function app(): express.Express {
  ...
  const distFolder = join(process.cwd(), 'dist/mail-app/browser');
  ...
  server.get('/api/send-email/:username', (req, res) => {
    ...
    request(`http://127.0.0.1:4200/welcome-email/${req.params.username}`, (error, response, body) => {
      ...
      res.send(`
          <h4>This code comes from WelcomeEmailComponentComponent.ts and this template is generated from server.ts for ${req.params.username}</h4>
          <hr/>
          <pre><code>${escapeHtml(body)}</code></pre>
          <hr/>
          <h4>Implement the email sending method and handle the response here.</h4>
      `);
